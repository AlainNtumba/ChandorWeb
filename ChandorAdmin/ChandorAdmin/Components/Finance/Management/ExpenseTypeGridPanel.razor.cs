...
public partial class ExpenseTypeGridPanel : IDisposable
{
    [Inject] IExpensesTypeService ExpensesTypeService { get; set; } = null!;

    SfDialog? _dialog;
    SfGrid<ExpensesTypeDto>? _gridRef;
    ...
    List<ExpensesTypeDto> _gridData = [];
    ...
    async Task LoadDataAsync()
    {
        try
        {
            var response = await ExpensesTypeService.GetAllExpensesTypesAsync();
    ...
    async Task OnActionBeginAsync(ActionEventArgs<ExpensesTypeDto> args)
    {
        ...
        try
        {
            ...
            {
                var request = new NewExpensesTypeDto
                {
                    Name = row.Name.Trim(),
                    Description = row.Description?.Trim() ?? string.Empty
                };

                var response = await ExpensesTypeService.CreateExpensesTypeAsync(request);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de dépense créé avec succès.");
                    await LoadDataAsync();
            ...
            {
                var response = await ExpensesTypeService.UpdateExpensesTypeAsync(row);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de dépense mis à jour avec succès.");
                    await LoadDataAsync();
                    if (_gridRef is not null)
                        await _gridRef.CloseEditAsync();
                }
    ...
    async Task OnToolbarClickAsync(ClickEventArgs args)
    {
        ...
        foreach (var record in selected)
        {
            try
            {
                var response = await ExpensesTypeService.DeleteExpensesTypeAsync(record.Id);
        ...
        if (allSucceeded)
            await NotifySuccessAsync("Type de dépense supprimé avec succès.");
        else
