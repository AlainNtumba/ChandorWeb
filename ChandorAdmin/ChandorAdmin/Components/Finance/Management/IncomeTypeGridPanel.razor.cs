...
public partial class IncomeTypeGridPanel : IDisposable
{
    [Inject] IIncomeTypeService IncomeTypeService { get; set; } = null!;

    SfDialog? _dialog;
    SfGrid<IncomeTypeDto>? _gridRef;
    ...
    async Task LoadDataAsync()
    {
        try
        {
            var response = await IncomeTypeService.GetAllIncomeTypesAsync();
    ...
    async Task OnActionBeginAsync(ActionEventArgs<IncomeTypeDto> args)
    {
        ...
        try
        {
            ...
            {
                var request = new NewIncomeTypeDto
                {
                    Name = row.Name.Trim(),
                    Description = row.Description?.Trim() ?? string.Empty
                };

                var response = await IncomeTypeService.CreateIncomeTypeAsync(request);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de revenu créé avec succès.");
                    await LoadDataAsync();
            ...
            {
                var response = await IncomeTypeService.UpdateIncomeTypeAsync(row);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de revenu mis à jour avec succès.");
                    await LoadDataAsync();
                    if (_gridRef is not null)
                        await _gridRef.CloseEditAsync();
                }
    ...
    async Task OnToolbarClickAsync(ClickEventArgs args)
    {
        ...
        foreach (var record in selected)
        {
            try
            {
                var response = await IncomeTypeService.DeleteIncomeTypeAsync(record.Id);
        ...
        if (allSucceeded)
            await NotifySuccessAsync("Type de revenu supprimé avec succès.");
        else
