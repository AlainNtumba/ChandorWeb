...
public partial class MemberRoleGridPanel : IDisposable
{
    [Inject] IMemberRoleService MemberRoleService { get; set; } = null!;

    SfDialog? _dialog;
    SfGrid<MemberRoleDto>? _gridRef;
    ...
    async Task OnActionBeginAsync(ActionEventArgs<MemberRoleDto> args)
    {
        ...
        try
        {
            ...
            {
                var request = new NewMemberRoleDto
                {
                    Name = row.Name.Trim(),
                    Description = row.Description?.Trim() ?? string.Empty
                };

                var response = await MemberRoleService.CreateMemberRoleAsync(request);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Rôle de membre créé avec succès.");
                    await LoadDataAsync();
            ...
            {
                var response = await MemberRoleService.UpdateMemberRoleAsync(row);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Rôle de membre mis à jour avec succès.");
                    await LoadDataAsync();
                    if (_gridRef is not null)
                        await _gridRef.CloseEditAsync();
                }
    ...
    async Task OnToolbarClickAsync(ClickEventArgs args)
    {
        ...
        foreach (var record in selected)
        {
            try
            {
                var response = await MemberRoleService.DeleteMemberRoleAsync(record.Id);
        ...
        if (allSucceeded)
            await NotifySuccessAsync("Rôle de membre supprimé avec succès.");
        else
