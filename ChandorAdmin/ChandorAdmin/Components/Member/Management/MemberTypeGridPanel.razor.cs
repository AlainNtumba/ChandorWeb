...
public partial class MemberTypeGridPanel : IDisposable
{
    [Inject] IMemberTypeService MemberTypeService { get; set; } = null!;

    SfDialog? _dialog;
    SfGrid<MemberTypeDto>? _gridRef;
    ...
    async Task OnActionBeginAsync(ActionEventArgs<MemberTypeDto> args)
    {
        ...
        try
        {
            ...
            {
                var request = new NewMemberTypeDto
                {
                    Name = row.Name.Trim(),
                    Description = row.Description?.Trim() ?? string.Empty
                };

                var response = await MemberTypeService.CreateMemberTypeAsync(request);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de membre créé avec succès.");
                    await LoadDataAsync();
            ...
            {
                var response = await MemberTypeService.UpdateMemberTypeAsync(row);
                if (response is { Success: true })
                {
                    await NotifySuccessAsync("Type de membre mis à jour avec succès.");
                    await LoadDataAsync();
                    if (_gridRef is not null)
                        await _gridRef.CloseEditAsync();
                }
    ...
    async Task OnToolbarClickAsync(ClickEventArgs args)
    {
        ...
        foreach (var record in selected)
        {
            try
            {
                var response = await MemberTypeService.DeleteMemberTypeAsync(record.Id);
        ...
        if (allSucceeded)
            await NotifySuccessAsync("Type de membre supprimé avec succès.");
        else
